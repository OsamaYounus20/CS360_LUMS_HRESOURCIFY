...
exports.add = async function(req, res) {
    if (req.body.Message === 'add user') {      // if the client sends data to insert a new value in the user table in the database
        user_password = generatePassword();     // generating a password for the new user

        // querying the database to get the number of users in order to generate the user_id
        connection.query('SELECT COUNT(*) FROM HRUSER', async function(error, results, fields) {
            if (error) {
                console.log(error);
            } else {
                // user_id is of the form 20xx
    ...
    } else {    // if the client requests a list of departments for the dropdown menu
        // querying the database to get all departments
        connection.query('SELECT dept_id, dept_name FROM DEPARTMENT', async function (error, results, fields) {
            if (error) {
